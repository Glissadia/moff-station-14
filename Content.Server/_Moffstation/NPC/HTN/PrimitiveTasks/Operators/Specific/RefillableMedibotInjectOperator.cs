...
public sealed partial class RefillableMedibotInjectOperator : HTNOperator
{
    ...
    public override HTNOperatorStatus Update(NPCBlackboard blackboard, float frameTime)
    {
        ...
        if (!_entMan.TryGetComponent<RefillableMedibotComponent>(owner, out var botComp))
            return HTNOperatorStatus.Failed;

        if (!_medibot.CheckInjectable((owner, botComp), target) || !_medibot.TryInject((owner, botComp), target))
            return HTNOperatorStatus.Failed;

        _chat.TrySendInGameICMessage(owner, Loc.GetString("refillable-medibot-finish-inject"), InGameICChatType.Speak, hideChat: true, hideLog: true);
